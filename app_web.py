...
def cargar_bases_web():
    """Descarga los datos de Google Sheets y unifica nombres de columnas para evitar errores."""
    try:
        ...
        # Normalización para evitar KeyError: 'id_consultante'
        for df in [df_c, df_p, df_t]:
            if not df.empty:
                df.columns = df.columns.str.strip()
                # Unificamos variantes de ID
...
# VISOR DE AUDITORÍA TÉCNICA (Lo que veías en la terminal de VS Code)
...
if modo_app == "📅 Programar Cliente":
    ...
    st.markdown("<p style='color: #B48E92;'>Selecciona un cliente de tu base de datos y asígnale un reporte.</p>", unsafe_allow_html=True)
    if not df_cli.empty:
        ...
        if st.button("➕ Programar", type="primary"):
            st.info("Para esta versión, agrega la fila en tu Google Sheet y recarga la página.")
...
# 6. MODO TALLER DE INFORMES (LÓGICA PRINCIPAL)
    ...
    if not st.session_state.textos_generados:
        ...
        st.markdown("<p style='color: #B48E92;'>Procesa efemérides y genera interpretaciones profesionales de alto impacto.</p>", unsafe_allow_html=True)

    # Filtrar solo informes con estado PENDIENTE
    ...
    if pendientes.empty and not st.session_state.textos_generados:
        st.sidebar.success("✅ ¡Todo al día! No hay pendientes.")
        ...
        for idx, row in pendientes.iterrows():
            if 'id_consultante' in row:
                ...
                # Detectar tipo de informe por ID para mostrarlo en el menú
        ...
        if opciones_menu:
            ...
            # Detectar el tipo de informe (1=Natal, 2=Tránsitos, 3=RS)
            id_t = str(row_p.get('Id_Informe', '1')).replace('.0', '').strip()

            # --- BLOQUE DE RELOCALIZACIÓN RS (RECUPERADO Y VISIBLE) ---
            lat_rs, lon_rs, lug_final = None, None, ""
            # Si el informe es tipo 3 o el texto de la selección dice Revolución
            if id_t == "3" or "Revolucion" in sel_p: 
                ...
                lug_rs_input = st.sidebar.text_input("Ciudad de Relocalización:", placeholder="Ej: Madrid, España", key="ciudad_rs_input")
                
                if st.sidebar.button("🔍 Buscar en Mapa", use_container_width=True):
                    if lug_rs_input:
                        with st.spinner("Buscando coordenadas exactas..."):
                            try:
                                geolocator = Nominatim(user_agent="astroimpacto_final_v4")
                                ...
                                if loc:
                                    ...
                                    st.sidebar.success(f"Ubicación encontrada: {loc.address}")
                                else:
                                    st.sidebar.error("Lugar no encontrado. Escribe Ciudad y País.")
                            except Exception:
                                st.sidebar.error("Error de conexión con el servidor de mapas.")
            ...
            if st.sidebar.button("🚀 PROCESAR INFORME", type="primary", use_container_width=True):
                with st.spinner("Calculando posiciones y redactando interpretación..."):
                    if id_t == "2":
                        ...
                        # PASAMOS EL LUGAR CONFIRMADO PARA QUE LA IA LO USE EN EL TEXTO
    ...
    if st.session_state.textos_generados:
        ...
        st.info("Modifica los textos si es necesario. Estos se guardarán directamente en el informe final.")
        ...
        if tipo == "REVOLUCION":
            ...
            with st.expander("2. Introducción y Bases (Natal/Transitos/Prog)", expanded=False):
                ...
                d['como_actuar_progresiones'] = st.text_area("Consejos de Progresiones (Uno por línea)", "\n".join(d.get('como_actuar_progresiones', []))).split("\n")
        ...
        # Preparar logo e icono antes de generar
        ...
        try:
            ...
            with c_final1:
                if st.button("👁️ VER VISTA PREVIA (PANTALLA)"):
                    try: 
