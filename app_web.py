...
<style>
    ...
    .stTextArea textarea { font-size: 0.95rem; line-height: 1.6; color: #333; }
...
# 2. INICIALIZACIÓN DE VARIABLES DE SESIÓN (MEMORIA)
if 'textos_generados' not in st.session_state:
    st.session_state.update({
        ...
        'tipo_reporte_actual': "NATAL", 'lat_rs_auto': "", 'lon_rs_auto': "", 'lugar_rs_confirmado': "", 'idx_prog_actual': None
    })
    ...
    try:
        ...
        for df in [df_c, df_p, df_t]:
            if not df.empty:
                ...
                for col_name in ['id', 'Id', 'ID', 'ID_CONSULTANTE']:
...
# 4. MENÚ DE NAVEGACIÓN Y VISOR TÉCNICO
...
if modo_app == "📅 Programar Cliente":
    ...
    if not df_cli.empty:
        ...
        if st.button("➕ Programar", type="primary"):
            st.info("Agrega la fila en tu Google Sheet y recarga la página.")
...
# 6. MODO TALLER (LÓGICA PRINCIPAL)
    ...
    if not st.session_state.textos_generados:
        ...
        st.markdown("<p style='color: #B48E92;'>Procesa efemérides e interpretaciones profesionales.</p>", unsafe_allow_html=True)
    ...
    if pendientes.empty and not st.session_state.textos_generados:
        ...
        if opciones_menu:
            ...
            # --- BUSCADOR DE COORDENADAS (CIUDAD RS) - VISIBLE ---
            ...
            if id_t == "3": 
                ...
                lug_rs_input = st.sidebar.text_input("Ciudad de la Revolución:", placeholder="Ej: Buenos Aires, AR")
                    ...
                    if lug_rs_input:
                        with st.spinner("Buscando..."):
                            try:
                                ...
                                else: st.sidebar.error("No se encontró.")
                            except Exception: st.sidebar.error("Error de mapa.")
    ...
    if st.session_state.textos_generados:
        ...
        if tipo == "REVOLUCION":
            with st.expander("1. Infografía (Perspectivas)", expanded=False):
                ...
                with col1:
                    d['perspectivas']['transformacion'] = st.text_area("Reto Transformación", d['perspectivas'].get('transformacion', ''), height=100)
                    d['perspectivas']['cambio'] = st.text_area("Área Cambio", d['perspectivas'].get('cambio', ''), height=100)
                with col2:
                    d['perspectivas']['oportunidades'] = st.text_area("Oportunidades", d['perspectivas'].get('oportunidades', ''), height=100)
                    d['perspectivas']['relaciones'] = st.text_area("Vínculos", d['perspectivas'].get('relaciones', ''), height=100)
            
            with st.expander("2. Introducción y Bases (Natal/Transitos/Prog)", expanded=False):
                d['intro_texto'] = st.text_area("Intro", d.get('intro_texto',''), height=100)
                d['carta_natal_resumen'] = st.text_area("Resumen Natal", d.get('carta_natal_resumen',''), height=150)
                d['transitos_personales'] = st.text_area("Tránsitos Lentos", d.get('transitos_personales',''), height=150)
                d['progresiones_secundarias'] = st.text_area("Progresiones", d.get('progresiones_secundarias',''), height=150)
                d['como_actuar_progresiones'] = st.text_area("Tips Progresiones (Separar con líneas)", "\n".join(d.get('como_actuar_progresiones', []))).split("\n")
            ...
            with st.expander("3. Revolución Solar (General y Laboral)", expanded=True):
                ...
                d['situacion_laboral_economica'] = st.text_area("Laboral", d.get('situacion_laboral_economica',''), height=150)

            with st.expander("4. Emocional, Trimestral y Cierre", expanded=True):
                d['situacion_emocional'] = st.text_area("Emocional", d.get('situacion_emocional',''), height=150)
            ...
            with st.expander("1. Energía Base", expanded=False):
                d['interpretacion_sol_signo'] = st.text_area("Sol", d.get('interpretacion_sol_signo',''), height=100)
                d['interpretacion_luna_signo'] = st.text_area("Luna", d.get('interpretacion_luna_signo',''), height=100)
                d['interpretacion_asc_signo'] = st.text_area("AC", d.get('interpretacion_asc_signo',''), height=100)
            
            with st.expander("2. Análisis del Clima Anual", expanded=True):
                d['frase_anual_corta'] = st.text_input("Lema", d.get('frase_anual_corta',''))
                d['analisis_clima_anual'] = st.text_area("Interpretación", d.get('analisis_clima_anual',''), height=300)
                d['oportunidad_anual'] = st.text_area("Oportunidad", d.get('oportunidad_anual',''), height=120)
                d['atencion_anual'] = st.text_area("Atención", d.get('atencion_anual',''), height=120)

            with st.expander("3. Calendario Mensual", expanded=True):
            ...
            with st.expander("1. Tríada Principal", expanded=True):
                ...
                d['interpretacion_asc_signo'] = st.text_area("Camino Ascendente", d.get('interpretacion_asc_signo',''), height=150)
            ...
            with st.expander("3. Síntesis y FODA", expanded=True):
                d['interpretacion_personalidad_global'] = st.text_area("Identidad Evolutiva", d.get('interpretacion_personalidad_global',''), height=350)
                f_c1, f_c2 = st.columns(2)
                with f_c1: d['foda']['fortalezas'] = st.text_area("Fortalezas", "\n".join(d['foda'].get('fortalezas', []))).split("\n")
                with f_c2: d['foda']['debilidades'] = st.text_area("Debilidades", "\n".join(d['foda'].get('debilidades', []))).split("\n")
        ...
        try:
            plant_f = env.get_template(st.session_state.plantilla_usar)
            with c_f1:
                if st.button("👁️ VER VISTA PREVIA"):
                    try: components.html(plant_f.render(d), height=900, scrolling=True)
                    except Exception: st.error("Error render.")
            with c_f2:
                html_f = plant_f.render(d)
                st.download_button("💾 DESCARGAR INFORME HTML", data=html_f, file_name=f"Informe_{d.get('nombre_cliente')}.html", mime="text/html", type="primary")
                if st.button("❌ FINALIZAR"):
                    st.session_state.textos_generados = False
                    st.rerun()
        except Exception as e: st.error(f"Plantilla no encontrada: {e}")
